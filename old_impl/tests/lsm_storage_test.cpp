...
TEST(LsmStorageTest, RangeScanTest) {
  LsmStorageOptions options = {1, 50};
  LsmStorageInner storage("test", options);
  for (int i = 5; i < 30; i++) {
    storage.Put(std::to_string(i), std::to_string(i));
  ...
  auto iter = storage.Scan("10", "20");
  for (int i = 10; i < 20; i++) {
    auto result = std::make_pair(iter->Key(), iter->Value());
    ASSERT_EQ(result.first, std::to_string(i));
    ASSERT_EQ(result.second, std::to_string(i));
    iter->Next();
  }
}
